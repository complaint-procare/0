...
const result = {
  ...
  edgeFunctions: [await checkFunction('upload-attachment')],
  storageBuckets: [await checkBucket('complaint-media')],
}
...
const failedBuckets = result.storageBuckets.filter((bucket) => !bucket.ok)

if (failedTables.length || failedFunctions.length || failedBuckets.length || !result.auth.ok) {
  process.exitCode = 2
}
