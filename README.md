...
| Secret | `SUPABASE_ANON_KEY` | `VITE_SUPABASE_ANON_KEY` у frontend build |
| Secret | `GOOGLE_OAUTH_CLIENT_ID` | `VITE_GOOGLE_OAUTH_CLIENT_ID` у frontend build |
...
CI виставляє Google OAuth secrets у Supabase перед деплоєм і деплоїть обидві Edge Functions: `upload-attachment` та `google-oauth-callback`.
...
1. `/settings/fields` — повний CRUD полів (типи, обовʼязковість, видимість у формах)
